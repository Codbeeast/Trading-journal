...
import { useTrades } from '../context/TradeContext'; // Import useTrades hook
...
const EliteTradingCalendar = () => {
  ...
  // Get tradeHistory (renamed from 'trades' in context to avoid confusion) from the centralized context
  ...
  const getCalendarWeeks = () => {
    ...
    let firstWeekdayOfMonth = firstDayOfMonth.getDay(); // 0 (Sun) to 6 (Sat)
    let daysToPrepend = 0;
    if (firstWeekdayOfMonth === 0) { // If 1st is Sunday, we need 5 empty days (Mon-Fri of prev week)
      daysToPrepend = 5;
    } else if (firstWeekdayOfMonth === 1) { // If 1st is Monday, 0 empty days
      daysToPrepend = 0;
    } else { // If 1st is Tue-Sat, prepend (firstWeekdayOfMonth - 1) days
    ...
    // Add trailing empty cells to fill the last week (ensure a multiple of 5 for full weeks)
  ...
  const formatDate = (date) => {
    if (!date) return ''; // Handle null dates for empty cells
  ...
  const navigateMonth = (direction) => {
    const newDate = new Date(currentDate.getFullYear(), currentDate.getMonth() + direction, 1); // Set day to 1
  ...
  const getTooltipPosition = (dayIndexInWeek) => {
    // For 5 columns, if it's the last column (index 4), position right, otherwise left.
    return dayIndexInWeek === 4 ? 'right-0' : 'left-0';
  };

  const renderCalendarDay = (dayInfo, weekIndex, dayIndexInWeek) => {
    if (dayInfo.isEmpty) {
      return (
        <div
    ...
    const tooltipPosition = getTooltipPosition(dayIndexInWeek);
    const isProfit = dayStats.totalPnl > 0;

    // Use green for profit, dark red for losses
    const dayColorClass = isProfit ?
      ...
      'from-red-900/40 via-gray-900/90 to-gray-800/80 shadow-lg shadow-red-700/20 border-2 border-red-800/60'; // Dark red for losses
    const textColorClass = isProfit ? 'text-green-400' : 'text-red-500'; // Dark red for losses
    const accentBgClass = isProfit ? 'bg-green-400/20' : 'bg-red-700/20'; // Dark red for losses
    ...
    return (
      ...
      >
          ...
          <div className="space-y-1 sm:space-y-2">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-1">
                ...
                <TrendingUp className='w-3 h-3 sm:w-4 sm:h-4 text-green-400' /> : <TrendingDown className='w-3 h-3 sm:w-4 sm:h-4 text-red-500'/>} {/* Red for negative pips */}
                <span className={`text-xs sm:text-sm font-semibold ${dayStats.totalPips >= 0 ? 'text-green-400' : 'text-red-500'}`}> {/* Red for negative pips */}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className={`w-2 h-2 sm:w-3 sm:h-3 ${isProfit ? 'bg-green-400' : 'bg-red-700'} rounded-full`}></div> {/* Dark red for non-profit dot */}
            </div>
          </div>
        )}
        ...
        {hoveredDate === dateString && (
          <div
            className={`absolute z-[9999] top-full ${tooltipPosition} mt-2 w-80 bg-gray-900/98 backdrop-blur-2xl border-2 ${isProfit ? 'border-green-500/60' : 'border-red-800/60'} rounded-2xl shadow-2xl ${isProfit ? 'shadow-green-500/20' : 'shadow-red-700/20'} p-3 transform transition-all duration-300 animate-slideIn`}
            style={{
              boxShadow: `
                0 20px 40px rgba(0, 0, 0, 0.5),
                0 0 40px ${isProfit ? 'rgba(34, 197, 94, 0.2)' : 'rgba(220, 38, 38, 0.2)'}, /* Red color for shadow */
                inset 0 2px 0 rgba(255, 255, 255, 0.1)
              `,
              position: 'fixed', // Use fixed to ensure it stays in view when scrolling
              zIndex: 9999,
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-2">
                <Calendar className={`w-4 h-4 ${isProfit ? 'text-green-400' : 'text-red-500'}`} /> {/* Consistent color */}
                <h4 className={`font-bold text-base ${isProfit ? 'text-green-400' : 'text-red-500'}`}> {/* Consistent color */}
                  {dayInfo.date.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
                </h4>
              </div>
              <span className={`text-xs text-white px-2 py-1 rounded-full backdrop-blur-sm font-semibold ${isProfit ? 'bg-green-600/80' : 'bg-red-700/80'}`}> {/* Consistent color */}
            ...
            <div className="grid grid-cols-3 gap-2 mb-3">
              <div className={`bg-gradient-to-br ${isProfit ? 'from-green-900/60 to-green-800/40 border-green-700/50' : 'from-red-900/60 to-red-800/40 border-red-700/50'} backdrop-blur-sm p-2 rounded-lg border`}>
                <div className="flex items-center space-x-1 mb-1">
                  <DollarSign className={`w-3 h-3 ${isProfit ? 'text-green-400' : 'text-red-400'}`} /> {/* Consistent color */}
                  <span className={`text-xs ${isProfit ? 'text-green-300' : 'text-red-300'} font-medium`}>P&L</span> {/* Consistent color */}
                </div>
              ...
              {dayStats.trades.map((trade, index) => (
                ...
                >
                  <div className="flex flex-col">
                    ...
                    <div className="flex items-center space-x-1">
                      <span className={`px-1 py-0.5 rounded text-xs font-bold ${trade.buySell === 'BUY' ? 'bg-green-600/80 text-white' : 'bg-red-600/80 text-white'}`}> {/* Red for SELL */}
                        {trade.buySell}
                      </span>
                  ...
                  <div className="text-right flex items-center space-x-2">
                    {/* Image or No Image message */}
                    {trade.uploadedImage ? (
                      <ImageIcon className="w-4 h-4 text-blue-400 cursor-pointer"
                          onClick={(e) => {
                              e.stopPropagation(); // Prevent opening trade details
                    ...
                    <div>
                        <div className={`font-bold text-sm ${(trade.pnl || 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}> {/* Red for negative PnL */}
  ...
  return (
    <div className="w-full min-h-auto bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 p-2 sm:p-4 lg:p-6">
      <div className="flex flex-col gap-4 lg:gap-6 max-w-7xl mx-auto"> {/* Added max-w-7xl mx-auto to control overall width */}
        ...
        {/* Calendar Header Section (Month Navigation, Title) */}
        ...
        {/* Main Calendar Grid and Weekly Summaries */}
        <div className="relative z-10 w-full grid grid-cols-1 md:grid-cols-[repeat(5,minmax(60px,1fr))_minmax(280px,320px)] gap-1 sm:gap-2">
          ...
          {/* Empty header for the summary column */}
          <div className="p-2 sm:p-3 text-center text-sm sm:text-base font-bold text-white bg-gradient-to-r from-gray-800/80 to-gray-700/60 backdrop-blur-xl rounded-lg border border-gray-600/50 shadow-lg hidden md:block">
            Weekly Performance
          </div>

          {/* Calendar weeks and their summaries */}
          {weeks.map((week, weekIndex) => {
            ...
            return (
              <React.Fragment key={weekIndex}>
                {/* Render 5 calendar days directly into the main grid */}
                {week.map((dayInfo, dayIndexInWeek) => renderCalendarDay(dayInfo, weekIndex, dayIndexInWeek))}

                {/* Render Weekly Summary for the week */}
