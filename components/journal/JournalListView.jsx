...
import { Calendar, Clock, DollarSign, TrendingUp, TrendingDown, Edit3, Trash2, ExternalLink } from 'lucide-react';

const JournalListView = ({ trades, onEdit, onDelete }) => {
    if (!trades || trades.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center p-12 bg-black/30 backdrop-blur-xl rounded-3xl border border-white/10">
                ...
                <h3 className="text-xl font-bold text-gray-300">No trades yet</h3>
                <p className="text-gray-500 mt-2">Switch to "New" mode and click "Add Trade" to get started.</p>
            </div>
        );
    }
                        ...
                        <div className={`p-6 bg-gradient-to-b ${bgGradient}`}>
                            ...
                            <div className="grid grid-cols-4 gap-y-4 gap-x-2 border-t border-white/5 py-4 mb-4">
                                <div>
                                    ...
                                    <div className="text-sm font-semibold text-gray-200 truncate">{trade.strategyName || trade.strategy || 'N/A'}</div>
                                </div>
                                <div>
                                    <div className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Entry Type</div>
                                    <div className="text-sm font-semibold text-gray-200 truncate" title={Array.isArray(trade.entryType) ? trade.entryType.join(', ') : trade.entryType}>{Array.isArray(trade.entryType) ? trade.entryType.join(', ') : trade.entryType || '-'}</div>
                                </div>
                            ...
                            <div className="bg-black/20 rounded-2xl p-4 border border-white/5 mb-6">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div className="text-[10px] text-gray-500 mt-2 uppercase tracking-wider mb-1">Setup</div>
                                        <div className="text-sm text-gray-300 truncate">{Array.isArray(trade.setupType) ? trade.setupType.join(', ') : trade.setupType || '-'}</div>
                                    </div>
                                    <div className="border-l border-white/10 pl-4">
                                        ...
                                        <div className="text-[10px] text-gray-500 mt-2 uppercase tracking-wider mb-1">TF Used</div>
                                        <div className="text-sm text-gray-300">{Array.isArray(trade.timeFrame) ? trade.timeFrame.join(', ') : trade.timeFrame || '-'}</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mb-6">
                                ...
                                <p className="text-sm text-gray-400 italic text-center line-clamp-2 px-4">
                                    "{trade.notes || 'No analysis notes provided for this trade.'}"
                                </p>
                            </div>
