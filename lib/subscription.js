...
export async function createTrialSubscription(userId, planType = '1_MONTH', username = null) {
    try {
        ...
        // TEST MODE: 10 Minutes Trial (was 7 days)
        trialEndDate.setMinutes(trialEndDate.getMinutes() + 10);
...
export async function getSubscriptionStatus(userId) {
    try {
        ...
        if (!subscription) {
            ...
            if (isEligible) {
                try {
                    console.log(`Starting automatic 10-minute trial for user ${userId}`);
                    subscription = await createTrialSubscription(userId, '1_MONTH', 10);
                } catch (err) {
        ...
        // TEMP: Auto-migrate to 10-min test trial
        if (subscription && subscription.isTrialActive && subscription.trialStartDate) {
            ...
            // Force update it to Start + 10 mins
            if ((currentEndDate - now) > (1000 * 60 * 60 * 3)) {
                console.log('Migrating user to 10-min test trial...');
                const newEndDate = new Date(trialStart);
                newEndDate.setMinutes(newEndDate.getMinutes() + 10);
