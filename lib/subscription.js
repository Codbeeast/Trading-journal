...
export async function getSubscriptionStatus(userId) {
    try {
        ...
        if (!subscription) {
            ...
            if (isEligible) {
                try {
                    console.log(`Starting automatic 7-day trial for user ${userId}`);
                ...
                // User is not eligible (has used trial), but getActiveSubscription returned null.
                // This means they likely have an EXPIRED trial.
                // We must fetch it so the migration logic below can check if it needs to be extended/corrected.
                subscription = await Subscription.findOne({ userId }).sort({ createdAt: -1 });
            }
        }

        // Auto-migrate/Correct trial to 7 days
        // This ensures the trial is always "Start Date + 7 Days"
        if (subscription && subscription.isTrialActive && subscription.trialStartDate) {
            ...
            // Calculate what the end date SHOULD be (Start + 7 days)
            ...
            if (diffInMs > 60000) { // If diff > 1 minute
                console.log(`Correcting trial duration to 7 days (Change from ${currentEndDate.toISOString()} to ${targetEndDate.toISOString()})`);

                subscription.trialEndDate = targetEndDate;
                // If now > targetEndDate, it will expire naturally
                await subscription.save();
                console.log('Correction complete. New end date:', targetEndDate);
            }
        }
