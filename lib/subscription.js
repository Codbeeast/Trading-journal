...
export async function createTrialSubscription(userId, planType = '1_MONTH', username = null) {
    try {
        ...
        // TEST MODE: 55 Minutes Trial (was 7 days)
        trialEndDate.setMinutes(trialEndDate.getMinutes() + 55);
...
export async function getSubscriptionStatus(userId) {
    try {
        ...
        if (!subscription) {
            ...
            if (isEligible) {
                try {
                    console.log(`Starting automatic 55-minute trial for user ${userId}`);
                    subscription = await createTrialSubscription(userId, '1_MONTH', 55);
                } catch (err) {
        ...
        // TEMP: Auto-migrate to 55-min test trial
        if (subscription && subscription.isTrialActive && subscription.trialStartDate) {
            ...
            // Force update it to Start + 55 mins
            if ((currentEndDate - now) > (1000 * 60 * 60 * 3)) {
                console.log('Migrating user to 55-min test trial...');
                const newEndDate = new Date(trialStart);
                newEndDate.setMinutes(newEndDate.getMinutes() + 55);
