...
export async function getSubscriptionStatus(userId) {
    try {
        ...
        // TEMP: Auto-migrate to 7-day trial if currently on a short test trial (e.g. < 2 hours)
        // This ensures the user gets their full trial back after testing
        if (subscription && subscription.isTrialActive && subscription.trialStartDate) {
            ...
            const totalDuration = currentEndDate - trialStart;

            // If trial is very short (less than 24 hours), migrate back to 7 days
            if (totalDuration < (1000 * 60 * 60 * 24)) {
                console.log('Migrating user back to full 7-day trial...');
                const newEndDate = new Date(trialStart);
                newEndDate.setDate(newEndDate.getDate() + 7);

                subscription.trialEndDate = newEndDate;
                subscription.status = 'trial'; // Ensure status is trial
                await subscription.save();
                console.log('Migration to 7-day trial complete. New end date:', newEndDate);
            }
        }
