...
export async function createTrialSubscription(userId, planType = '1_MONTH', username = null) {
    try {
        ...
        // TEST MODE: 25 Minutes Trial (was 7 days)
        trialEndDate.setMinutes(trialEndDate.getMinutes() + 25);
...
export async function getSubscriptionStatus(userId) {
    try {
        ...
        if (!subscription) {
            ...
            if (isEligible) {
                try {
                    console.log(`Starting automatic 25-minute trial for user ${userId}`);
                    subscription = await createTrialSubscription(userId, '1_MONTH', 25);
                } catch (err) {
        ...
        // TEMP: Auto-migrate to 25-min test trial
        // This ensures the trial is always "Start Date + 25 mins" regardless of previous changes
        if (subscription && subscription.isTrialActive && subscription.trialStartDate) {
            ...
            // Calculate what the end date SHOULD be (Start + 25 mins)
            const targetEndDate = new Date(trialStart);
            targetEndDate.setMinutes(targetEndDate.getMinutes() + 25);

            // Check if stored end date differs from target by more than 1 minute
            // This handles ANY previous duration (55m, 10m, 7 days, etc.)
            const diffInMs = Math.abs(currentEndDate - targetEndDate);

            if (diffInMs > 60000) { // If diff > 1 minute
                console.log(`Migrating user to 25-min trial (Correction form ${currentEndDate.toISOString()} to ${targetEndDate.toISOString()})`);

                subscription.trialEndDate = targetEndDate;
                // If now > targetEndDate, it will expire naturally
                await subscription.save();
                console.log('Migration complete. New end date:', targetEndDate);
            }
        }
        ...
        if (isInTrial) {
            ...
            if (diff > 0) {
                ...
                if (days > 0) {
                    ...
                    // Use Ceil to ensure it shows "10m" at start and "1m" until the very end (avoids "0m")
                    const mins = Math.ceil((diff % (1000 * 60 * 60)) / (1000 * 60));
                    timeRemainingString = `${mins}m`;
                }
            } else {
